...
export class Command {
	...
	async execute(): Promise<void> {
		...
		const dependenciesToInstall = await this.getDependenciesToInstall.execute(formatter, linter, selectedConfigs)
		...
		const HuskyConfigConfirm = AutoApproveFlag
			? AutoApproveFlag
			: await Confirm({
					message: 'You want to set up Husky pre-commit hooks to run your formatter and linter before each commit?',
				})
