...
export class Repository implements IRepository {
	...
	private collectFormatterPlugins(techs: string[], formatter: string): string[] {
		...
		for (const tech of techs) {
			const config = configs.techs[tech as keyof typeof configs.techs]
			const formatterConfig = (config?.formatter as Record<string, { plugins?: string[] }>)?.[formatter]
	...
	private collectFileExtensions(techs: string[]): string[] {
		...
		for (const tech of techs) {
			const config = configs.techs[tech as keyof typeof configs.techs]
			const eslintConfig = config?.linter?.eslint as { config?: { fileExtensions?: string[] } } | undefined
	...
	async writeLintStagedConfig(formatter: Formatters, linter: Linters, techs: string[]): Promise<void> {
	...
	async getEditorExtensions(formatter: Formatters, linter: Linters): Promise<{ name: string; vsc: string; vsx: string }[]> {
