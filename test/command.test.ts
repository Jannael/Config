...
describe('Command.execute', () => {
  ...
  test('runs the full happy path in the correct order', async () => {
    ...
    record(
      'useCase.writeFormatterConfig',
      useCases.writeFormatterConfig.execute as unknown as AnyMock,
    )
  ...
  test('skips the initial installDependencies but still writes configs when user declines install', async () => {
    ...
    const installCalls = (repo.installDependencies as unknown as { mock: { calls: unknown[][] } })
      .mock.calls
  ...
  test('skips editor and husky configuration when user declines them', async () => {
    Confirm.mockImplementation(
      (async ({ message }: { message: string }) =>
        !(message.includes('editor') || message.includes('Husky'))) as never,
    )
  ...
  test('passes the selected configs to linter and formatter use-cases', async () => {
    ...
    expect(useCases.writeLinterConfig.execute).toHaveBeenCalledWith('eslint', [
      'astro',
      'typescript',
    ])
    expect(useCases.writeFormatterConfig.execute).toHaveBeenCalledWith('prettier', [
      'astro',
      'typescript',
    ])
  })
  ...
  test('husky is initialized before lint-staged config is written', async () => {
    ...
    const lintStagedOrder = (repo.writeLintStagedConfig as unknown as AnyMock).mock
