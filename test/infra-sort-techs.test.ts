...
describe('Repository.sortTechs (eslint parser order)', () => {
  ...
  test('puts known base techs first (javascript, typescript) in their declared order', () => {
    expect(sortTechs(['react', 'typescript', 'javascript'])).toEqual([
      'javascript',
      'typescript',
      'react',
    ])
  ...
  test('mixes known and unknown techs correctly: known first in declared order, then unknown in input order', () => {
    expect(sortTechs(['astro', 'react', 'typescript', 'html'])).toEqual([
      'typescript',
      'html',
      'astro',
      'react',
  ...
  test('handles tailwind and html in declared order, with framework techs after', () => {
    expect(sortTechs(['react', 'tailwind', 'html', 'typescript'])).toEqual([
      'typescript',
      'html',
      'tailwind',
      'react',
...
describe('Repository.collectEslintConfig (integration: order propagates to imports/spreads)', () => {
  test('astro parser/import comes AFTER typescript in importStatements and configSpread', () => {
    ...
    const tsImportIdx = cfg.importStatements.indexOf(
      "import typescriptEslint from 'typescript-eslint'",
    )
    const astroImportIdx = cfg.importStatements.indexOf(
  ...
  test('vue parser comes AFTER typescript in importStatements', () => {
    ...
    const vueImportIdx = cfg.importStatements.indexOf(
      "import eslintPluginVue from 'eslint-plugin-vue'",
    )
    const vueParserIdx = cfg.importStatements.indexOf(
  ...
  test('deduplicates import statements when the same plugin appears in multiple techs', () => {
    ...
    const tsImports = cfg.importStatements.filter(
      (s) => s === "import typescriptEslint from 'typescript-eslint'",
    )
